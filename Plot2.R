...
balt_ann_emissions <- with(baltimore_MD, tapply(Emissions, year, sum))
#NICE COLOURS
rb <- colorRampPalette(c("Light Blue", "Blue"))
dat_rb <- rb(4)[cut(balt_ann_emissions, breaks = 4)]
#PLOT
png("plot2.png", width = 480, height = 480)
barplot(balt_ann_emissions, 
        xlab = "year", 
        ylab = expression("Total PM"[2.5]*" Emitted (ton)"), 
        main = "Baltimore State: Total Emissions By Year",
        col  = dat_rb)
dev.off()
